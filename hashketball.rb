...
# location = home or away, team_data = list of each player /stats for location
  game_hash.each do | location, team_data |
    ...
    #drill into the next layer, attribute = :team_name, data = team name
    team_data.each do | attribute, data |
      
      if attribute == :players
        
        # drill down to a player name and their stats. Check to see if the name key matches the player name that was passed in (player_name)...name = player name, statistics = his hash of stats 
